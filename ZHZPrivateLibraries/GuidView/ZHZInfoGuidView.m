...
@implementation ZHZInfoGuidView
...
- (void)applyDefaultsToSelfDuringInitializationWithFrame:(CGRect)frame pages:(NSArray *)pagesArray {
    ...
    _skipButtonAlignment = ZHZViewAlignmentRight;
    _skipped = NO;
    _limitPageIndex = -1;
...
- (ZHZInfoPage *)pageForIndex:(NSUInteger)idx {
    if(idx >= _pages.count) {
        return nil;
    }
    return (ZHZInfoPage *)_pages[idx];
}
...
- (void)checkIndexForScrollView:(ZHZInfoGuidScrollView *)scrollView {
...
- (void)notifyDelegateWithPreviousPage:(NSUInteger)previousPageIndex andCurrentPage:(NSUInteger)currentPageIndex {
    if(currentPageIndex!=_currentPageIndex && currentPageIndex < _pages.count) {
        ZHZInfoPage* previousPage = _pages[previousPageIndex];
        ZHZInfoPage* currentPage = _pages[currentPageIndex];
...
- (ZHZInfoGuidScrollView *)scrollView {
    if (!_scrollView) {
        _scrollView = [[ZHZInfoGuidScrollView alloc] initWithFrame:self.bounds];
...
- (UIButton *)skipButton {
    if (!_skipButton) {
        ...
        [_skipButton setTitle:NSLocalizedString(@"开启***之旅", nil) forState:UIControlStateNormal];
...
- (UIView *)viewForPage:(ZHZInfoPage *)page atXIndex:(CGFloat)xIndex {
...
- (NSString*)accessibilityLabelForPage:(ZHZInfoPage*)page
{
...
- (void)buildFooterView {
    ...
    if (self.skipButton && !self.skipButton.hidden) {
        if(self.skipButtonAlignment == ZHZViewAlignmentCenter) {
        ...
        } else if(self.skipButtonAlignment == ZHZViewAlignmentLeft) {
        ...
        } else if(self.skipButtonAlignment == ZHZViewAlignmentRight) {
...
- (void)scrollViewWillBeginDragging:(ZHZInfoGuidScrollView *)scrollView {
...
- (void)scrollViewDidEndDecelerating:(ZHZInfoGuidScrollView *)scrollView {
...
- (void)scrollViewDidEndScrollingAnimation:(ZHZInfoGuidScrollView *)scrollView {
    [self checkIndexForScrollView:scrollView];
}

- (void)scrollViewDidScroll:(ZHZInfoGuidScrollView *)scrollView {
...
- (void)deviceOrientationDidChange:(NSNotification *)notification {
    ...
    NSUInteger i = 0;
    for (ZHZInfoPage *page in _pages) {
...
- (void)setSkipButtonAlignment:(ZHZViewAlignment)skipButtonAlignment {
...
- (void)showInView:(UIView *)view animateDuration:(CGFloat)duration withInitialPageIndex:(NSUInteger)initialPageIndex {
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        ZHZInfoPage *currentPage = _pages[self.currentPageIndex];
        if(currentPage.onPageDidAppear) currentPage.onPageDidAppear();
