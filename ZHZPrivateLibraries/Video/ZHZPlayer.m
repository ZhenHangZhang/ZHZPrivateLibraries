...
@interface ZHZPlayer ()<UIGestureRecognizerDelegate>
...
-(void)fullScreenAction:(UIButton *)sender{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ZHZPlayerFullScreenButtonClickedNotification object:sender];
}
-(void)colseTheVideo:(UIButton *)sender{
    [self.player pause];
    [[NSNotificationCenter defaultCenter] postNotificationName:ZHZPlayerClosedNotification object:sender];
}
- (double)duration{
...
- (void)handleSingleTap{
    [[NSNotificationCenter defaultCenter] postNotificationName:ZHZPlayerSingleTapNotification object:nil];
...
- (void)handleDoubleTap{
    [[NSNotificationCenter defaultCenter] postNotificationName:ZHZPlayerDoubleTapNotification object:nil];
    if (self.player.rate != 1.f) {
        if ([self currentTime] == self.duration)
            [self setCurrentTime:0.f];
        [self.player play];
...
- (void)finishedPlay:(NSTimer *)timer{
    if (self.currentTime == self.duration&&self.player.rate==.0f) {
        self.playOrPauseBtn.selected = YES;
        //播放完成后的通知
        [[NSNotificationCenter defaultCenter] postNotificationName:ZHZPlayerFinishedPlayNotification object:self.durationTimer];
