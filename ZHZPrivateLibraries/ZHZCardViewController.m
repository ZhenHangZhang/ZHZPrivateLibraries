...
@interface ZHZCardViewController ()<ZHZDragCardDelegate>
...
@implementation ZHZCardViewController
...
-(void)loadAllCards{
    ...
    for (int i=0; i<self.allCards.count; i++) {
        ZHZCardAnimotion *draggableView=self.allCards[i];
    ...
    for (int i=0; i<_allCards.count ;i++) {
        ...
        [UIView animateKeyframesWithDuration:0.5 delay:0.06*i options:UIViewKeyframeAnimationOptionCalculationModeLinear animations:^{
            ...
            if (i>0&&i<CARD_NUM-1) {
                ZHZCardAnimotion *preDraggableView=[_allCards objectAtIndex:i-1];
...
-(void)refreshAllCards{
    ...
    for (int i=0; i<_allCards.count ;i++) {
        ...
        ZHZCardAnimotion *card=self.allCards[i];
...
-(void)addCards{
    for (int i = 0; i<CARD_NUM; i++) {
        ...
        ZHZCardAnimotion *draggableView = [[ZHZCardAnimotion alloc]initWithFrame:CGRectMake([[UIScreen mainScreen]bounds].size.width+CARD_WIDTH, self.view.center.y-CARD_HEIGHT/2, CARD_WIDTH, CARD_HEIGHT)];
...
-(void)swipCard:(ZHZCardAnimotion *)cardView Direction:(BOOL)isRight {
    ...
    for (int i = 0; i<CARD_NUM; i++) {
        ZHZCardAnimotion*draggableView=[_allCards objectAtIndex:i];
        draggableView.originalCenter=draggableView.center;
        draggableView.originalTransform=draggableView.transform;
        if (i==0) {
            draggableView.canPan=YES;
...
-(void)moveCards:(CGFloat)distance{
    ...
    if (fabs(distance)<=PAN_DISTANCE) {
        for (int i = 1; i<CARD_NUM-1; i++) {
            ZHZCardAnimotion *draggableView=_allCards[i];
            ZHZCardAnimotion *preDraggableView=[_allCards objectAtIndex:i-1];
...
-(void)rightButtonClickAction {
    ...
    _flag = YES;
    ZHZCardAnimotion *dragView=self.allCards[0];
