...
typedef enum {
    ...
    ZHZMenuViewArrowDirectionNone,
    ZHZMenuViewArrowDirectionUp,
    ZHZMenuViewArrowDirectionDown,
    ZHZMenuViewArrowDirectionLeft,
    ZHZMenuViewArrowDirectionRight,
    
} ZHZMenuViewArrowDirection;

@interface ZHZMenuView : UIView
...
@interface ZHZMenuOverlay : UIView

@end
@interface ZHZCustomMenu ()
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[ZHZMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
@implementation ZHZMenuView

{
    ZHZMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    ZHZMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([ZHZCustomMenu hasShadow]) {
            self.layer.shadowOpacity = 0.5;
            self.layer.shadowColor = [ZHZCustomMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = ZHZMenuViewArrowDirectionUp;
        ...
        _arrowDirection = ZHZMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = ZHZMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _arrowDirection = ZHZMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = ZHZMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(ZHZMenuSelectedItem)selectedItem {
    ...
    ZHZMenuOverlay *overlay = [[ZHZMenuOverlay alloc] initWithFrame:view.bounds];
...
- (void)dismissMenu:(BOOL)animated {
    if (self.superview) {
        if (animated) {
            ...
            [UIView animateWithDuration:0.2 animations:^(void) {
                ...
                if ([self.superview isKindOfClass:[ZHZMenuOverlay class]])
    ...
    [ZHZCustomMenu sharedMenu].isShow = NO;
}

- (void)performAction:(id)sender {
    [self dismissMenu:YES];
    UIButton *button = (UIButton *)sender;
    ZHZCustomMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView {
    ...
    UIFont *titleFont = [ZHZCustomMenu titleFont];
    
    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;
    
    for (ZHZCustomMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [ZHZMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [ZHZMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == ZHZMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == ZHZMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == ZHZMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == ZHZMenuViewArrowDirectionRight) {
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [ZHZCustomMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([ZHZCustomMenu backgrounColorEffect] == ZHZMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == ZHZMenuViewArrowDirectionLeft ||
        _arrowDirection == ZHZMenuViewArrowDirectionRight) {
...
/// MenuView
static ZHZCustomMenu                      *gMenu;
/// 背景色
static UIColor                      *gTintColor;
/// 字体
static UIFont                       *gTitleFont;
/// 背景色效果
static ZHZMenuBackgrounColorEffect   gBackgroundColorEffect = ZHZMenuBackgrounColorEffectSolid;
/// 是否显示阴影
static BOOL                          gHasShadow = NO;

@implementation ZHZCustomMenu{
    ZHZMenuView *_menuView;
    BOOL         _observing;
}
...
+ (instancetype)sharedMenu {
    ...
    dispatch_once(&onceToken, ^{
        gMenu = [[ZHZCustomMenu alloc] init];
    });
    return gMenu;
}
...
+ (void)reset {
    gTintColor = nil;
    gTitleFont = nil;
    gBackgroundColorEffect = ZHZMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
}
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(ZHZMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[ZHZMenuView alloc] init];
