...
#import "ZHZImageScrollViewLoop.h"
...
@interface ZHZImageScrollViewLoop ()<UIScrollViewDelegate>
{
    NSTimer *_timer;
    int _pageNumber;
}
...
@implementation ZHZImageScrollViewLoop
...
-(void)setImageArray:(NSArray *)imageArray{
    ...
    for(int i = 0 ; i < imageArray.count; i++){
        ...
        NSString *picUrl = [imageArray[i] objectForKey:@"pic"];
        //        picUrl = [NSString convertImgStr:picUrl];//当前工程需要截取url
        
        NSString *imageName =[NSString stringWithFormat:@"%@",picUrl];
        NSLog(@"%@",imageName);
